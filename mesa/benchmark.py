...
def benchmark():
    ...
    time_single_check_empty = timeit.repeat(lambda: single_grid.is_cell_empty((0, 0)), number=1000, repeat=10)

    # Getting a list of all empty cells (SingleGrid)
    time_single_list_empty = timeit.repeat(lambda: list(single_grid.get_all_empty_cells()), number=100, repeat=10)
    ...
    time_single_move_agent = timeit.repeat(move_single_agent, number=1000, repeat=10)

    # MultiGrid Benchmarks
    multi_grid, multi_agents = setup_multi_grid()

    # Check if a specific cell is empty (MultiGrid)
    time_multi_check_empty = timeit.repeat(lambda: multi_grid.is_cell_empty((0, 0)), number=1000, repeat=10)

    # Getting a list of all empty cells (MultiGrid)
    time_multi_list_empty = timeit.repeat(lambda: list(multi_grid.get_all_empty_cells()), number=100, repeat=10)
    ...
    time_multi_move_agent = timeit.repeat(move_multi_agent, number=1000, repeat=10)
    ...
    time_multi_iterate_agents = timeit.repeat(iterate_multi_agents, number=1000, repeat=10)
